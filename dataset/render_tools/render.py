...
from utils.render_utils import (
    ...
    render_rgb_image,
    render_depth_map,
    render_sem_ins_seg_map,
    add_background_color_for_image,
    get_camera_pos_mat,
    merge_joint_qpos,
)
...
def render_one_image(
    ...
    # 读取urdf文件中的关节结构信息
    joints_dict = read_joints_from_urdf_file(
        data_path, "mobility_annotation_gapartnet.urdf"
    )
    ...
    camera_pos = get_cam_pos(
        theta_min=camera_range["theta_min"],
    ...
    sem_seg_map, ins_seg_map, valid_linkName_to_instId = render_sem_ins_seg_map(
        scene=scene, camera=camera, link_pose_dict=link_pose_dict, depth_map=depth_map
    )
    valid_link_pose_dict = {
        link_name: link_pose_dict[link_name]
        for link_name in valid_linkName_to_instId.keys()
    }

    # 8. acquire camera intrinsic and extrinsic matrix
    # 获取相机矩阵信息
    camera_intrinsic, world2camera_rotation, camera2world_translation = (
        get_camera_pos_mat(camera)
    )

    # 9. calculate NPCS map
    # 映射到规范坐标空间
    valid_linkPose_RTS_dict, valid_NPCS_map = get_NPCS_map_from_oriented_bbox(
        depth_map,
        ins_seg_map,
    ...
    if replace_texture:
        ...
        ), "Texture replacement is only needed for PartNet dataset"
        texture_joints_dict = read_joints_from_urdf_file(
            data_path, "mobility_texture_gapartnet.urdf"
        )
        texture_joint_qpos = merge_joint_qpos(
            joint_qpos, joints_dict, texture_joints_dict
        )
        scene, camera, engine, robot = set_all_scene(
            data_path=data_path,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset", type=str, default="partnet", help="Specify the dataset to render"
    )
    parser.add_argument(
        "--model_id", type=int, default=100392, help="Specify the model id to render"
    )
    parser.add_argument(
        "--camera_idx",
        type=int,
        default=0,
        help="Specify the camera range index to render",
    )
    parser.add_argument(
        "--render_idx", type=int, default=0, help="Specify the render index to render"
    )
    parser.add_argument(
        "--height",
        type=int,
        default=800,
        help="Specify the height of the rendered image",
    )
    parser.add_argument(
        "--width", type=int, default=800, help="Specify the width of the rendered image"
    )
    parser.add_argument(
        "--ray_tracing",
        type=bool,
    ...
    if args.dataset == "akb48":
        assert (
            not args.replace_texture
        ), "Texture replacement is not needed for AKB48 dataset"
