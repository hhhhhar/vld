...
def set_all_scene(
    ...
    for joint in joints:
        ...
        if (
            joint_type == "revolute"
            or joint_type == "prismatic"
            or joint_type == "continuous"
        ):
...
# 构建映射，可视id->实例名，实例名->类别id，用于语义分割
def get_visid2gapart_mapping_dict(
    scene: sapien.Scene, linkId2catName, target_parts_list
):
    ...
    for articulation in scene.get_all_articulations():
        for link in articulation.get_links():
            ...
            for visual in link.get_visual_bodies():
                ...
                if (
                    visual_name.find("handle") != -1
                    and linkId2catName[link_id].find("handle") == -1
                ):
                    # visial name handle; link name not handle: fixed handle!
                    inst_name = (
                        link_name
                        + ":"
                        + linkId2catName[link_id]
                        + "/"
                        + visual_name.split("-")[0]
                        + ":"
                        + "fixed_handle"
                    ...
                    if inst_name not in instName2catId.keys():
                        instName2catId[inst_name] = (
                            target_parts_list.index("fixed_handle") + 1
                        )
                    ...
                    if inst_name not in instName2catId.keys():
                        instName2catId[inst_name] = (
                            target_parts_list.index(linkId2catName[link_id]) + 1
                        )
                else:
                    inst_name = "others"
                    visual_id = visual.get_visual_id()
...
# 渲染语义和实例分割图
def render_sem_ins_seg_map(
    scene: sapien.Scene, camera, link_pose_dict, depth_map, eps=1e-6
):
    vis_id_to_link_name = {}
    for articulation in scene.get_all_articulations():
        for link in articulation.get_links():
    ...
    seg_labels_by_visual_id = seg_labels[..., 0].astype(
        np.uint16
    )  # H x W, save each pixel's visual id
    height, width = seg_labels_by_visual_id.shape

    sem_seg_map = np.ones((height, width), dtype=np.int32) * (
        -1
    )  # -2 for background, -1 for others, 0~N-1 for N categories
    ins_seg_map = np.ones((height, width), dtype=np.int32) * (
        -1
    )  # -2 for background, -1 for others, 0~M-1 for M instances
...
def backproject_depth_into_pointcloud(
    depth_map, ins_seg_map, valid_linkName_to_instId, camera_intrinsic, eps=1e-6
):
    part_pcs_dict = {}
    ...
    for link_name, inst_id in valid_linkName_to_instId.items():
        ...
        for y, x in zip(ys, xs):
            ...
            x_proj = (
                (float(x) - camera_intrinsic[0, 2]) * z_proj / camera_intrinsic[0, 0]
            )
            y_proj = (
                (float(y) - camera_intrinsic[1, 2]) * z_proj / camera_intrinsic[1, 1]
            )
